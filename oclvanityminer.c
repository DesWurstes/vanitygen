...
static int workitem_cmp(workitem_t *a, workitem_t *b) {
  ...
  if (!cmpres)
    cmpres = (a->reward < b->reward) ? -1 : ((a->reward > b->reward) ? 1 : 0);
  return cmpres;
}
...
static pubkeybatch_t *pubkeybatch_avl_search(avl_root_t *rootp,
  ...
  avl_item_t *itemp = rootp->ar_root;
  pubkey_hex =
      EC_POINT_point2hex(pgroup, pubkey, POINT_CONVERSION_UNCOMPRESSED, NULL);
  while (itemp) {
    int cmpres;
...
static pubkeybatch_t *pubkeybatch_avl_insert(avl_root_t *rootp,
					     pubkeybatch_t *vpnew) {
  pubkeybatch_t *vp;
  avl_item_t *itemp = NULL;
  avl_item_t **ptrp = &rootp->ar_root;
...
static int server_pubkeybatch_equal(server_context_t *ctxp, pubkeybatch_t *a,
				    pubkeybatch_t *b) {
  workitem_t *wipa, *wipb;

  if (a->nitems != b->nitems) return 0;
  if (EC_POINT_cmp(EC_KEY_get0_group(ctxp->dummy_key), a->pubkey, b->pubkey,
		   NULL))
    return 0;
...
int server_workitem_add(server_request_t *reqp, workitem_t *wip) {
  ...
  if (pbatch == NULL) {
    ...
    pbatch->pubkey_hex = EC_POINT_point2hex(
	reqp->group, wip->pubkey, POINT_CONVERSION_UNCOMPRESSED, NULL);
  ...
  if (wip->pubkey && wip->pubkey != pbatch->pubkey) EC_POINT_free(wip->pubkey);
...
static size_t server_body_reader(const char *buf, size_t elemsize, size_t len,
				 void *param) {
  ...
  if ((reqp->part_size < (reqp->part_end + len + 1)) && (reqp->part_off > 0)) {
  ...
  while (1) {
    ...
    *sep = '\0';
    ...
    wip =
	server_workitem_new(reqp, pfx, pubkey_s, addrtype_s, reward_s, comment);
...
int server_context_submit_solution(server_context_t *ctxp, workitem_t *work,
				   const char *privkey) {
  char urlbuf[8192];
  char *pubhex;
  CURL *creq;
...
void output_match_work_complete(vg_context_t *vcp, EC_KEY *pkey,
				const char *pattern, int nothing) {
...
pubkeybatch_t *most_valuable_pkb(server_context_t *scp) {
  ...
  for (pbatch = pubkeybatch_avl_first(&scp->items); pbatch != NULL;
       pbatch = pubkeybatch_avl_next(pbatch)) {
    if (!res || (res->total_value < pbatch->total_value)) res = pbatch;
  }
  return res;
}
...
void usage(const char *name) {
  fprintf(
      ...
      "Organized vanity address mining client using OpenCL.  Contacts the "
      "specified\n"
      "bounty pool server, downloads a list of active bounties, and attempts "
      "to\n"
      "generate the address with the best difficulty to reward ratio.  "
      "Maintains\n"
      "contact with the bounty pool server and periodically refreshes the "
      "bounty\n"
      "list.\n"
      ...
      "reported.  To use multiple devices simultaneously, specify the -D "
      "option "
      "for\n"
      "each device.\n"
      "\n"
...
int main(int argc, char **argv) {
  ...
  if (verbose > 0) {
    fprintf(stderr, "WARNING: Built with " OPENSSL_VERSION_TEXT
		    "\n"
  ...
  if (ndevstrs) {
    for (opt = 0; opt < ndevstrs; opt++) {
      vocp = vg_ocl_context_new_from_devstr(vcp, devstrs[opt], safe_mode,
					    verify_mode);
      if (!vocp) {
    ...
    vocp =
	vg_ocl_context_new(vcp, platformidx, deviceidx, safe_mode, verify_mode,
