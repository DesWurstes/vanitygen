...
static workitem_t * workitem_avl_search(
	avl_root_t * rootp, const char * pattern) {
	workitem_t * vp;
...
static workitem_t * workitem_avl_insert(
	avl_root_t * rootp, workitem_t * vpnew) {
	workitem_t * vp;
...
static workitem_t * server_workitem_new(server_request_t * reqp,
	const char * pfx, const char * pubkey_s, const char * addrtype_s,
	const char * reward_s, const char * comment) {
	workitem_t * wip;
	EC_POINT * pubkey;
	int addrtype;
	double reward;
...
static int server_pubkeybatch_equal(
	...
	if (a->nitems != b->nitems)
		return 0;
	if (EC_POINT_cmp(
						EC_KEY_get0_group(ctxp->dummy_key), a->pubkey, b->pubkey, NULL))
		return 0;
...
server_context_t * server_context_new(
	const char * url, const char * credit_addr) {
...
void usage(const char * name) {
	fprintf(stderr,
		...
		"reported.  To use multiple devices simultaneously, specify the -D option "
		"for\n"
		"each device.\n"
		"\n"
		"Options:\n"
...
int main(int argc, char ** argv) {
	...
	if (ndevstrs) {
		for (opt = 0; opt < ndevstrs; opt++) {
			vocp =
				vg_ocl_context_new_from_devstr(vcp, devstrs[opt], safe_mode, verify_mode);
			if (!vocp) {
