...
void vg_exec_downgrade_lock() {}

int vg_exec_upgrade_lock() { return 0; }

/*
 * OpenCL debugging and support
 */

static const char *vg_ocl_strerror(cl_int ret) {
...
static const char *vg_ocl_platform_getstr(
	cl_platform_id pid, cl_platform_info param) {
	static char platform_str[1024];
	cl_int ret;
	size_t size_ret;
	ret = clGetPlatformInfo(
		pid, param, sizeof(platform_str), platform_str, &size_ret);
	if (ret != CL_SUCCESS) {
		snprintf(platform_str, sizeof(platform_str),
			"clGetPlatformInfo(%d): %s", param,
			vg_ocl_strerror(ret));
	}
	return platform_str;
}
...
static cl_platform_id vg_ocl_device_getplatform(cl_device_id did) {
	...
	size_t size_ret;
	ret = clGetDeviceInfo(
		did, CL_DEVICE_PLATFORM, sizeof(val), &val, &size_ret);
	if (ret != CL_SUCCESS) {
...
static cl_device_type vg_ocl_device_gettype(cl_device_id did) {
	...
	size_t size_ret;
	ret = clGetDeviceInfo(
		did, CL_DEVICE_TYPE, sizeof(val), &val, &size_ret);
	if (ret != CL_SUCCESS) {
...
static const char *vg_ocl_device_getstr(
	cl_device_id did, cl_device_info param) {
	static char device_str[1024];
	cl_int ret;
	size_t size_ret;
	ret = clGetDeviceInfo(
		did, param, sizeof(device_str), device_str, &size_ret);
	if (ret != CL_SUCCESS) {
...
static size_t vg_ocl_device_getsizet(cl_device_id did, cl_device_info param) {
	...
	if (ret != CL_SUCCESS) {
		fprintf(stderr, "clGetDeviceInfo(%d): %s", param,
			vg_ocl_strerror(ret));
	}
	return val;
}
...
void vg_ocl_dump_info(vg_ocl_context_t *vocp) {
	...
	fprintf(stderr, "Global memory: %llu\n",
		(unsigned long long) vg_ocl_device_getulong(
			did, CL_DEVICE_GLOBAL_MEM_SIZE));
	//	fprintf(stderr, "Max allocation: %ld\n",
	// This is long long unsigned on Mac and long unsigned on Linux
	fprintf(stderr, "Max allocation: %llu\n",
		(unsigned long long) vg_ocl_device_getulong(did,
			(unsigned long long) CL_DEVICE_MAX_MEM_ALLOC_SIZE));
	vocp->voc_dump_done = 1;
}

void vg_ocl_error(vg_ocl_context_t *vocp, int code, const char *desc) {
	...
	if (vocp && vocp->voc_ocldid) vg_ocl_dump_info(vocp);
}
...
static void vg_ocl_buildlog(vg_ocl_context_t *vocp, cl_program prog) {
	...
	ret = clGetProgramBuildInfo(prog, vocp->voc_ocldid,
		CL_PROGRAM_BUILD_LOG, 0, NULL, &logbufsize);
	if (ret != CL_SUCCESS) {
	...
	ret = clGetProgramBuildInfo(prog, vocp->voc_ocldid,
		CL_PROGRAM_BUILD_LOG, logbufsize, log, &logsize);
	if (ret != CL_SUCCESS) {
		vg_ocl_error(NULL, ret, "clGetProgramBuildInfo");
...
enum { VG_OCL_DEEP_PREPROC_UNROLL = (1 << 0),
	...
	case 0x1002: /* AMD/ATI */
		...
		if (vg_ocl_device_gettype(vocp->voc_ocldid) &
			...
			dvn = vg_ocl_device_getstr(
				vocp->voc_ocldid, CL_DEVICE_EXTENSIONS);
			if (dvn && strstr(dvn, "cl_amd_media_ops"))
				quirks |= VG_OCL_AMD_BFI_INT;

			dvn = vg_ocl_device_getstr(
				vocp->voc_ocldid, CL_DEVICE_NAME);
...
static int vg_ocl_create_kernel(
	vg_ocl_context_t *vocp, int knum, const char *func) {
	int i;
	cl_kernel krn;
	cl_int ret;
...
static void vg_ocl_hash_program(vg_ocl_context_t *vocp, const char *opts,
...
static int vg_ocl_amd_patch(
	vg_ocl_context_t *vocp, unsigned char *binary, size_t size) {
	...
	offset = 1;
	while (offset < (size - 8)) {
		ptr = (unsigned char *) memchr(
			binary + offset, 0x7f, size - offset);
...
static int vg_ocl_load_program(vg_context_t *vcp, vg_ocl_context_t *vocp,
	...
	if (!kfp) {
		...
		prog = clCreateProgramWithSource(
			vocp->voc_oclctx, 1, (const char **) &buf, &sz, &ret);
	} else {
		if (vcp->vc_verbose > 1)
		...
		fclose(kfp);
	rebuild:
		prog = clCreateProgramWithBinary(vocp->voc_oclctx, 1,
			&vocp->voc_ocldid, &szr, (const unsigned char **) &buf,
			&sts, &ret);
	}
	free(buf);
	if (!prog) {
	...
	if (fromsource && !(vocp->voc_quirks & VG_OCL_NO_BINARIES)) {
		ret = clGetProgramInfo(
			prog, CL_PROGRAM_BINARY_SIZES, sizeof(szr), &szr, &sz);
		if (ret != CL_SUCCESS) {
		...
		ret = clGetProgramInfo(
			prog, CL_PROGRAM_BINARIES, sizeof(buf), &buf, &sz);
		if (ret != CL_SUCCESS) {
			vg_ocl_error(vocp, ret,
...
static void CL_CALLBACK vg_ocl_context_callback(const char *errinfo,
...
static int vg_ocl_init(vg_context_t *vcp, vg_ocl_context_t *vocp,
	cl_device_id did, int safe_mode) {
	cl_int ret;
	char optbuf[128];
	int end = 0;
	...
	vocp->voc_oclctx = clCreateContext(
		NULL, 1, &did, vg_ocl_context_callback, NULL, &ret);
	if (!vocp->voc_oclctx) {
	...
	if (safe_mode) vocp->voc_quirks &= ~VG_OCL_OPTIMIZATIONS;
...
static int vg_ocl_kernel_arg_alloc(
	vg_ocl_context_t *vocp, int slot, int arg, size_t size, int host) {
	cl_mem clbuf;
	cl_int ret;
	int i, j, knum, karg;
	...
	clbuf = clCreateBuffer(vocp->voc_oclctx,
		CL_MEM_READ_WRITE | (host ? CL_MEM_ALLOC_HOST_PTR : 0), size,
		NULL, &ret);
	if (!clbuf) {
	...
	for (i = 0; i < MAX_SLOT; i++) {
		...
		for (j = 0; vg_ocl_arg_map[arg][j] >= 0; j += 2) {
			...
			ret = clSetKernelArg(vocp->voc_oclkernel[i][knum], karg,
				sizeof(clbuf), &clbuf);

			if (ret) {
				fprintf(stderr, "clSetKernelArg(%d,%d): ", knum,
					karg);
				vg_ocl_error(vocp, ret, NULL);
				return 0;
			}
...
int vg_ocl_copyout_arg(
	vg_ocl_context_t *vocp, int wslot, int arg, void *buffer, size_t size) {
	cl_int slot, ret;
...
static void *vg_ocl_map_arg_buffer(
	vg_ocl_context_t *vocp, int slot, int arg, int rw) {
	void *buf;
	cl_int ret;
...
static void vg_ocl_unmap_arg_buffer(
	vg_ocl_context_t *vocp, int slot, int arg, void *buf) {
	cl_int ret;
	cl_event ev;
	...
	ret = clEnqueueUnmapMemObject(vocp->voc_oclcmdq,
		vocp->voc_args[slot][arg], buf, 0, NULL, &ev);
	if (ret != CL_SUCCESS) {
...
int vg_ocl_kernel_int_arg(
	vg_ocl_context_t *vocp, int slot, int arg, int value) {
	cl_int ret;
	int i;
	...
	for (i = 0; i < MAX_SLOT; i++) {
		if ((i != slot) && (slot >= 0)) continue;
		ret = clSetKernelArg(
			vocp->voc_oclkernel[i][2], arg, sizeof(value), &value);
		if (ret) {
...
int vg_ocl_kernel_buffer_arg(
	vg_ocl_context_t *vocp, int slot, int arg, void *value, size_t size) {
	cl_int ret;
	int i, j, knum, karg;
...
static int vg_ocl_kernel_start(
	vg_ocl_context_t *vocp, int slot, int ncol, int nrow, int invsize) {
	cl_int val, ret;
	cl_event ev;
	size_t globalws[2] = { (long unsigned int) ncol,
	...
	val = invsize;
	ret = clSetKernelArg(
		vocp->voc_oclkernel[slot][1], 1, sizeof(val), &val);
	if (ret != CL_SUCCESS) {
		vg_ocl_error(vocp, ret, "clSetKernelArg(ncol)");
		return 0;
	}
	ret = clEnqueueNDRangeKernel(vocp->voc_oclcmdq,
		vocp->voc_oclkernel[slot][0], 2, NULL, globalws, NULL, 0, NULL,
		&ev);
	if (ret != CL_SUCCESS) {
	...
	ret = clEnqueueNDRangeKernel(vocp->voc_oclcmdq,
		vocp->voc_oclkernel[slot][1], 1, NULL, &invws, NULL, 0, NULL,
		&ev);
	if (ret != CL_SUCCESS) {
	...
	ret = clEnqueueNDRangeKernel(vocp->voc_oclcmdq,
		vocp->voc_oclkernel[slot][2], 2, NULL, globalws, NULL, 0, NULL,
		&ev);
	if (ret != CL_SUCCESS) {
...
static INLINE void vg_ocl_get_bignum_raw(BIGNUM *bn, const unsigned char *buf) {
#if OPENSSL_VERSION_NUMBER >= 0x0010100000
...
static INLINE void vg_ocl_put_bignum_raw(unsigned char *buf, const BIGNUM *bn) {
#if OPENSSL_VERSION_NUMBER >= 0x0010100000
...
static void vg_ocl_get_bignum_tpa(
	BIGNUM *bn, const unsigned char *buf, int cell) {
	...
	start = (((cell / ACCESS_STRIDE) * ACCESS_BUNDLE) +
		(cell % ACCESS_STRIDE));
	for (i = 0; i < 8; i++)
		memcpy(bnbuf + (i * 4), buf + 4 * (start + i * ACCESS_STRIDE),
			4);
...
static INLINE void vg_ocl_get_point(EC_POINT *ppnt, const unsigned char *buf) {
	static const unsigned char mont_one[] = { 0x01, 0x00, 0x00, 0x03,
		0xd1 };
...
static INLINE void vg_ocl_put_point(unsigned char *buf, const EC_POINT *ppnt) {
	assert(ppnt->Z_is_one);
	vg_ocl_put_bignum_raw(buf, PPNT_ARROW_X);
	vg_ocl_put_bignum_raw(buf + 32, PPNT_ARROW_Y);
}

static void vg_ocl_put_point_tpa(
	unsigned char *buf, int cell, const EC_POINT *ppnt) {
	...
	for (i = 0; i < 8; i++)
		memcpy(buf + 4 * (start + i * ACCESS_STRIDE), pntbuf + (i * 4),
			4);
	for (i = 0; i < 8; i++)
		memcpy(buf +
				4 *
...
static void vg_ocl_get_point_tpa(
	EC_POINT *ppnt, const unsigned char *buf, int cell) {
	unsigned char pntbuf[64];
	int start, i;
	...
	start = ((((2 * cell) / ACCESS_STRIDE) * ACCESS_BUNDLE) +
		(cell % (ACCESS_STRIDE / 2)));
	for (i = 0; i < 8; i++)
		memcpy(pntbuf + (i * 4), buf + 4 * (start + i * ACCESS_STRIDE),
			4);
	for (i = 0; i < 8; i++)
...
void show_elapsed(struct timeval *tv, const char *place) {
...
static int vg_ocl_gethash_check(vg_ocl_context_t *vocp, int slot) {
	...
	ocl_hashes_out =
		(unsigned char *) vg_ocl_map_arg_buffer(vocp, slot, 0, 0);
...
static int vg_ocl_prefix_rekey(vg_ocl_context_t *vocp) {
	...
	for (i = 0; i < vocp->voc_nslots; i++) {
		ocl_found_out =
			(uint32_t *) vg_ocl_map_arg_buffer(vocp, i, 0, 1);
		if (!ocl_found_out) {
			fprintf(stderr,
	...
	if (vocp->voc_pattern_rewrite) {
		...
		/* Write range records */
		ocl_targets_in =
			(unsigned char *) vg_ocl_map_arg_buffer(vocp, 0, 5, 1);
		if (!ocl_targets_in) {
			fprintf(stderr,
 ...
 * and the hardware.
 */
static int vg_ocl_verify_temporary(
	vg_ocl_context_t *vocp, int slot, int z_inverted) {
	vg_exec_context_t *vxcp = &vocp->base;
	...
	BIGNUM *bnz = BN_new(), *bnez = BN_new(), *bnm = BN_new(),
	       *bnzc = BN_new();
	BN_CTX *bnctx = NULL;
	BN_MONT_CTX *bnmont;
	int ret = 0;
	...
	int x, y, bx;
	static const unsigned char raw_modulus[] = { 0xFF, 0xFF, 0xFF, 0xFF,
	...
	ocl_points_in =
		(unsigned char *) vg_ocl_map_arg_buffer(vocp, slot, 3, 0);
	ocl_strides_in =
		(unsigned char *) vg_ocl_map_arg_buffer(vocp, slot, 4, 0);
...
out:
	if (z_heap) vg_ocl_unmap_arg_buffer(vocp, slot, 1, z_heap);
	if (point_tmp) vg_ocl_unmap_arg_buffer(vocp, slot, 2, point_tmp);
	if (ocl_points_in)
		vg_ocl_unmap_arg_buffer(vocp, slot, 3, ocl_points_in);
	if (ocl_strides_in)
		vg_ocl_unmap_arg_buffer(vocp, slot, 4, ocl_strides_in);
	if (ppr) EC_POINT_free(ppr);
	if (ppc) EC_POINT_free(ppc);
	if (pps) EC_POINT_free(pps);
	if (ppt) EC_POINT_free(ppt);
	BN_clear_free(bnz);
	BN_clear_free(bnez);
	BN_clear_free(bnm);
	if (bnmont) BN_MONT_CTX_free(bnmont);
...
static void *vg_opencl_loop(vg_exec_context_t *arg) {
	...
	EC_POINT_mul(
		pgroup, poffset, vxcp->vxc_bntmp, NULL, NULL, vxcp->vxc_bnctx);
	 ...
	 * - The row point array
	 */
	if (!vg_ocl_kernel_arg_alloc(
		    vocp, -1, 1, round_up_pow2(32 * 2 * round, 4096), 0) ||
		!vg_ocl_kernel_arg_alloc(
			vocp, -1, 2, round_up_pow2(32 * 2 * round, 4096), 0) ||
		!vg_ocl_kernel_arg_alloc(
			vocp, -1, 3, round_up_pow2(32 * 2 * ncols, 4096), 1))
		goto enomem;
...
l_rekey:
	...
	for (i = 1; i < ncols; i++) {
		EC_POINT_add(pgroup, ppbase[i], ppbase[i - 1], pgen,
			vxcp->vxc_bnctx);
	}
	...
	for (i = 1; i < nrows; i++) {
		EC_POINT_add(pgroup, pprow[i], pprow[i - 1], pbatchinc,
			vxcp->vxc_bnctx);
	}
	...
	while (1) {
		...
		if ((npoints + round) < rekey_at) {
			if (npoints > 1) {
				...
				for (i = 0; i < nrows; i++) {
					EC_POINT_add(pgroup, pprow[i], pprow[i],
						poffset, vxcp->vxc_bnctx);
				}

				EC_POINTs_make_affine(
					pgroup, nrows, pprow, vxcp->vxc_bnctx);
			}

			/* Copy the row stride array to the device */
			ocl_strides_in =
				(unsigned char *) vg_ocl_map_arg_buffer(
					vocp, slot, 4, 1);
			if (!ocl_strides_in) {
				fprintf(stderr,
...
static void show_devices(cl_device_id *ids, int nd, int base) {
	...
	for (i = 0; i < nd; i++) {
		res = clGetDeviceInfo(
			ids[i], CL_DEVICE_NAME, sizeof(nbuf), nbuf, &len);
		if (res != CL_SUCCESS) continue;
		if (len >= sizeof(nbuf)) len = sizeof(nbuf) - 1;
		nbuf[len] = '\0';
		res = clGetDeviceInfo(
			ids[i], CL_DEVICE_VENDOR, sizeof(vbuf), vbuf, &len);
		if (res != CL_SUCCESS) continue;
...
void show_platforms(cl_platform_id *ids, int np, int base) {
	...
	for (i = 0; i < np; i++) {
		res = clGetPlatformInfo(
			ids[i], CL_PLATFORM_NAME, sizeof(nbuf), nbuf, &len);
		if (res != CL_SUCCESS) {
		...
		nbuf[len] = '\0';
		res = clGetPlatformInfo(
			ids[i], CL_PLATFORM_VENDOR, sizeof(vbuf), vbuf, &len);
		if (res != CL_SUCCESS) {
...
vg_ocl_context_t *vg_ocl_context_new(vg_context_t *vcp, int platformidx,
	...
	full_threads = vg_ocl_device_getsizet(
		vocp->voc_ocldid, CL_DEVICE_MAX_COMPUTE_UNITS);
	...
	if (!ncols) {
		memsize = vg_ocl_device_getulong(
			vocp->voc_ocldid, CL_DEVICE_GLOBAL_MEM_SIZE);
		allocsize = vg_ocl_device_getulong(
			vocp->voc_ocldid, CL_DEVICE_MAX_MEM_ALLOC_SIZE);
		memsize /= 2;
		ncols = full_threads;
		nrows = 2;
		...
		while ((!worksize || ((wsmult * 2) <= worksize)) &&
			((((unsigned) ncols) * ((unsigned) nrows) * 2 * 128) <
				memsize) &&
			((((unsigned) ncols) * ((unsigned) nrows) * 2 * 64) <
				allocsize)) {
			if (ncols > nrows)
				nrows *= 2;
			else
...
vg_ocl_context_t *vg_ocl_context_new_from_devstr(
	...
	*part2 = '\0';
	...
	return vg_ocl_context_new(vcp, platformidx, deviceidx, safemode, verify,
