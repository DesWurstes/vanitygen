...
static const char * vg_ocl_platform_getstr(
	...
	size_t size_ret;
	ret =
		clGetPlatformInfo(pid, param, sizeof(platform_str), platform_str, &size_ret);
	if (ret != CL_SUCCESS) {
		snprintf(platform_str, sizeof(platform_str), "clGetPlatformInfo(%d): %s",
...
static cl_platform_id vg_ocl_device_getplatform(cl_device_id did) {
	...
	if (ret != CL_SUCCESS) {
		fprintf(
			stderr, "clGetDeviceInfo(CL_DEVICE_PLATFORM): %s", vg_ocl_strerror(ret));
	}
	return val;
}
...
static const char * vg_ocl_device_getstr(
	...
	if (ret != CL_SUCCESS) {
		snprintf(device_str, sizeof(device_str), "clGetDeviceInfo(%d): %s", param,
			vg_ocl_strerror(ret));
	}
	return device_str;
}
...
void vg_ocl_dump_info(vg_ocl_context_t * vocp) {
	...
	fprintf(stderr, "Max allocation: %llu\n",
		(unsigned long long) vg_ocl_device_getulong(
			did, (unsigned long long) CL_DEVICE_MAX_MEM_ALLOC_SIZE));
...
static void vg_ocl_buildlog(vg_ocl_context_t * vocp, cl_program prog) {
	...
	ret = clGetProgramBuildInfo(
		prog, vocp->voc_ocldid, CL_PROGRAM_BUILD_LOG, 0, NULL, &logbufsize);
	if (ret != CL_SUCCESS) {
	...
	ret = clGetProgramBuildInfo(
		prog, vocp->voc_ocldid, CL_PROGRAM_BUILD_LOG, logbufsize, log, &logsize);
	if (ret != CL_SUCCESS) {
		vg_ocl_error(NULL, ret, "clGetProgramBuildInfo");
...
enum {
	...
	VG_OCL_OPTIMIZATIONS = (VG_OCL_DEEP_PREPROC_UNROLL | VG_OCL_PRAGMA_UNROLL |
		VG_OCL_EXPENSIVE_BRANCHES | VG_OCL_DEEP_VLIW | VG_OCL_AMD_BFI_INT),
...
static int vg_ocl_load_program(vg_context_t * vcp, vg_ocl_context_t * vocp,
	...
	if (!kfp) {
		fprintf(
			stderr, "Error loading kernel file '%s': %s\n", filename, strerror(errno));
		free(buf);
		return 0;
	}
	...
	snprintf(bin_name, sizeof(bin_name),
		...
		prog_hash[10], prog_hash[11], prog_hash[12], prog_hash[13], prog_hash[14],
		prog_hash[15]);
	...
	if (!kfp) {
		...
		fclose(kfp);
	rebuild:
		prog = clCreateProgramWithBinary(vocp->voc_oclctx, 1, &vocp->voc_ocldid, &szr,
	...
	if ((ret != CL_SUCCESS) || ((vcp->vc_verbose > 1) && fromsource && !patched)) {
...
static void CL_CALLBACK vg_ocl_context_callback(
	const char * errinfo, const void * private_info, size_t cb, void * user_data) {
	fprintf(stderr, "vg_ocl_context_callback error: %s\n", errinfo);
}

static int vg_ocl_init(
	vg_context_t * vcp, vg_ocl_context_t * vocp, cl_device_id did, int safe_mode) {
	cl_int ret;
	char optbuf[128];
	int end = 0;
	...
	vocp->voc_oclctx =
		clCreateContext(NULL, 1, &did, vg_ocl_context_callback, NULL, &ret);
	if (!vocp->voc_oclctx) {
	...
	vocp->voc_oclcmdq =
		clCreateCommandQueueWithProperties(vocp->voc_oclctx, &vocp->voc_ocldid,
#else
	vocp->voc_oclcmdq = clCreateCommandQueue(vocp->voc_oclctx, vocp->voc_ocldid,
#endif
			0, &ret);
	if (!vocp->voc_oclcmdq) {
	...
	if (vocp->voc_quirks & VG_OCL_DEEP_PREPROC_UNROLL)
		end += snprintf(optbuf + end, sizeof(optbuf) - end, "-DDEEP_PREPROC_UNROLL ");
	...
	if (vocp->voc_quirks & VG_OCL_EXPENSIVE_BRANCHES)
			...
			snprintf(optbuf + end, sizeof(optbuf) - end, "-DVERY_EXPENSIVE_BRANCHES ");
...
static int vg_ocl_kernel_arg_alloc(
	...
	clbuf = clCreateBuffer(vocp->voc_oclctx,
		CL_MEM_READ_WRITE | (host ? CL_MEM_ALLOC_HOST_PTR : 0), size, NULL, &ret);
	if (!clbuf) {
	...
	for (i = 0; i < MAX_SLOT; i++) {
		...
		for (j = 0; vg_ocl_arg_map[arg][j] >= 0; j += 2) {
			...
			ret =
				clSetKernelArg(vocp->voc_oclkernel[i][knum], karg, sizeof(clbuf), &clbuf);
...
static void * vg_ocl_map_arg_buffer(
	...
	buf = clEnqueueMapBuffer(vocp->voc_oclcmdq, vocp->voc_args[slot][arg], CL_TRUE,
		(rw == 2) ? (CL_MAP_READ | CL_MAP_WRITE) : (rw ? CL_MAP_WRITE : CL_MAP_READ),
...
static void vg_ocl_unmap_arg_buffer(
	...
	ret = clEnqueueUnmapMemObject(
		vocp->voc_oclcmdq, vocp->voc_args[slot][arg], buf, 0, NULL, &ev);
	if (ret != CL_SUCCESS) {
...
static INLINE void vg_ocl_get_bignum_raw(
	BIGNUM * bn, const unsigned char * buf) {
#if OPENSSL_VERSION_NUMBER >= 0x0010100000
...
static INLINE void vg_ocl_put_bignum_raw(
	unsigned char * buf, const BIGNUM * bn) {
#if OPENSSL_VERSION_NUMBER >= 0x0010100000
...
static INLINE void vg_ocl_get_point(
	EC_POINT * ppnt, const unsigned char * buf) {
...
static INLINE void vg_ocl_put_point(
	unsigned char * buf, const EC_POINT * ppnt) {
...
static int vg_ocl_gethash_init(vg_ocl_context_t * vocp) {
	...
	for (i = 0; i < vocp->voc_nslots; i++) {
		...
		if (!vg_ocl_kernel_arg_alloc(
							vocp, i, 0, 20 * vocp->voc_ocl_rows * vocp->voc_ocl_cols, 1))
			return 0;
	}
...
static int vg_ocl_verify_temporary(
	...
	int x, y, bx;
	static const unsigned char raw_modulus[] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
	...
	for (y = 0; y < vocp->voc_ocl_rows; y++) {
		...
		for (x = 0; x < vocp->voc_ocl_cols; x++) {
			...
			if (BN_cmp(PPT_ARROW_X, PPS_ARROW_X) || BN_cmp(PPT_ARROW_Y, PPS_ARROW_Y) ||
				BN_cmp(bnz, bnzc)) {
				if (!mismatches) {
...
static void * vg_opencl_loop(vg_exec_context_t * arg) {
	...
	if (!vg_ocl_kernel_arg_alloc(
		...
		!vg_ocl_kernel_arg_alloc(vocp, -1, 3, round_up_pow2(32 * 2 * ncols, 4096), 1))
		goto enomem;
...
#define BN_MASK2 (0xffffffffL)
	...
	if (vcp->vc_pubkey_base) {
		EC_POINT_add(
			pgroup, ppbase[0], ppbase[0], vcp->vc_pubkey_base, vxcp->vxc_bnctx);
	}
...
vg_ocl_context_t * vg_ocl_context_new(vg_context_t * vcp, int platformidx,
	...
	if (!nthreads) {
		...
		if (vg_ocl_device_gettype(vocp->voc_ocldid) & CL_DEVICE_TYPE_CPU)
			...
			nthreads =
				vg_ocl_device_getsizet(vocp->voc_ocldid, CL_DEVICE_MAX_WORK_GROUP_SIZE);
	}

	full_threads =
		vg_ocl_device_getsizet(vocp->voc_ocldid, CL_DEVICE_MAX_COMPUTE_UNITS);
	...
	if (!ncols) {
		...
		allocsize =
			vg_ocl_device_getulong(vocp->voc_ocldid, CL_DEVICE_MAX_MEM_ALLOC_SIZE);
		memsize /= 2;
		ncols = full_threads;
		nrows = 2;
