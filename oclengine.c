...
static const char *vg_ocl_platform_getstr(cl_platform_id pid,
...
static const char *vg_ocl_device_getstr(cl_device_id did,
...
enum {
  ...
  VG_OCL_OPTIMIZATIONS =
      (VG_OCL_DEEP_PREPROC_UNROLL | VG_OCL_PRAGMA_UNROLL |
...
static int vg_ocl_create_kernel(vg_ocl_context_t *vocp, int knum,
				const char *func) {
  int i;
  cl_kernel krn;
  cl_int ret;
...
static int vg_ocl_amd_patch(vg_ocl_context_t *vocp, unsigned char *binary,
			    size_t size) {
...
static int vg_ocl_kernel_arg_alloc(vg_ocl_context_t *vocp, int slot, int arg,
				   size_t size, int host) {
  cl_mem clbuf;
  cl_int ret;
  int i, j, knum, karg;
...
static void *vg_ocl_map_arg_buffer(vg_ocl_context_t *vocp, int slot, int arg,
				   int rw) {
  void *buf;
  cl_int ret;
...
int vg_ocl_kernel_int_arg(vg_ocl_context_t *vocp, int slot, int arg,
			  int value) {
  cl_int ret;
  int i;
...
int vg_ocl_kernel_buffer_arg(vg_ocl_context_t *vocp, int slot, int arg,
...
static int vg_ocl_kernel_start(vg_ocl_context_t *vocp, int slot, int ncol,
			       int nrow, int invsize) {
  cl_int val, ret;
  cl_event ev;
...
static void vg_ocl_get_bignum_tpa(BIGNUM *bn, const unsigned char *buf,
				  int cell) {
...
static void vg_ocl_put_point_tpa(unsigned char *buf, int cell,
				 const EC_POINT *ppnt) {
...
static void vg_ocl_get_point_tpa(EC_POINT *ppnt, const unsigned char *buf,
				 int cell) {
  unsigned char pntbuf[64];
  int start, i;
 ...
 * and the hardware.
 */
static int vg_ocl_verify_temporary(vg_ocl_context_t *vocp, int slot,
				   int z_inverted) {
  vg_exec_context_t *vxcp = &vocp->base;
...
out:
  ...
  if (ocl_points_in) vg_ocl_unmap_arg_buffer(vocp, slot, 3, ocl_points_in);
  if (ocl_strides_in) vg_ocl_unmap_arg_buffer(vocp, slot, 4, ocl_strides_in);
...
static void *vg_opencl_loop(vg_exec_context_t *arg) {
  ...
  for (i = 0; i < nslots; i++) {
     ...
     * - Column point array
     */
    if (!vg_ocl_kernel_arg_alloc(vocp, i, 4, 32 * 2 * nrows, 1)) goto enomem;
  }
   ...
   * - The row point array
   */
  if (!vg_ocl_kernel_arg_alloc(vocp, -1, 1, round_up_pow2(32 * 2 * round, 4096),
			       0) ||
      !vg_ocl_kernel_arg_alloc(vocp, -1, 2, round_up_pow2(32 * 2 * round, 4096),
			       0) ||
      !vg_ocl_kernel_arg_alloc(vocp, -1, 3, round_up_pow2(32 * 2 * ncols, 4096),
			       1))
    goto enomem;
...
vg_ocl_context_t *vg_ocl_context_new(vg_context_t *vcp, int platformidx,
  ...
  if (!ncols) {
    memsize =
	vg_ocl_device_getulong(vocp->voc_ocldid, CL_DEVICE_GLOBAL_MEM_SIZE);
    allocsize =
