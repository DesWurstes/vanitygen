...
void vg_output_match_console(vg_context_t *vcp, EC_KEY *pkey,
  ...
  if (vcp->vc_pubkey_base) {
    ...
    EC_POINT_add(EC_KEY_get0_group(pkey), ppnt, ppnt, vcp->vc_pubkey_base,
		 NULL);
    free_ppnt = 1;
    keytype = "PrivkeyPart";
  } else {
  ...
  if (isscript)
    vg_encode_script_address(ppnt, EC_KEY_get0_group(pkey), vcp->vc_istestnet,
			     addr2_buf);
  else {
    if (isaddresscompressed) {
      vg_encode_compressed_address(ppnt, EC_KEY_get0_group(pkey),
				   vcp->vc_istestnet, addr_buf);
    } else {
      vg_encode_address(ppnt, EC_KEY_get0_group(pkey), vcp->vc_istestnet,
			addr_buf);
    }
  }
...
int vg_context_add_patterns(vg_context_t *vcp, const char **const patterns,
			    int npatterns) {
...
static int get_prefix_ranges(int addrtype, const char *pfx, BIGNUM **result) {
  ...
  if (addrtype == 8) {
    if (pfx[0] != 'p') {
      fprintf(stderr,
	      "The first character of the prefix must be 'p' for "
	      "script addresses.\n");
      ret = -2;
      goto out;
    }
    ...
    if (pfx[0] != 'q') {
      fprintf(stderr,
	      "The first character of the prefix must be 'q' for "
	      "standard addresses.\n");
      ret = -2;
      goto out;
    }
  ...
  if (pfx[1] != 'q' && pfx[1] != 'p' && pfx[1] != 'z' && pfx[1] != 'r') {
    fprintf(stderr,
	    "The second character of the prefix must be either 'q', "
	    "'p', 'r' or 'z'.\n");
    ret = -2;
    goto out;
  }
...
static vg_prefix_t *vg_prefix_avl_insert(avl_root_t *rootp,
					 vg_prefix_t *vpnew) {
  vg_prefix_t *vp;
...
static vg_prefix_t *vg_prefix_add(avl_root_t *rootp, const char *pattern,
...
research:
  ...
  if (vp) {
	...
	vg_prefix_context_next_difficulty(vcpp, vxcp->vxc_bntmp,
					  vxcp->vxc_bntmp2, vxcp->vxc_bnctx);
...
inline static void vg_prefix_context_exec_prep(vg_context_t *vcp,
					       vg_exec_context_t *vxcp) {
  (void)vcp;
  (void)vxcp;
}
...
static int vg_regex_compile_patterns(const char **const patterns,
  ...
  for (unsigned int i = 0; i < (unsigned)npatterns; i++) {
    ids[i] = i;
    flags[i] = HS_FLAG_SINGLEMATCH | HS_FLAG_PREFILTER | HS_FLAG_CASELESS |
	       HS_FLAG_DOTALL;
  }
  return hs_compile_multi(patterns, flags, ids, npatterns, HS_MODE_BLOCK, NULL,
			  db, compile_error);
}
...
static int vg_regex_context_add_patterns(vg_context_t *vcp,
  ...
  if (vg_regex_compile_patterns(vcrp->vcr_regex_pat,
				npatterns + vcrp->base.vc_npatterns,
...
static void vg_regex_context_exec_prep(vg_context_t *vcp,
				       vg_exec_context_t *vxcp) {
  vg_regex_context_t *vcrp = (vg_regex_context_t *)vcp;
