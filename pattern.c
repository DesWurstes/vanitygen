...
void vg_exec_context_calc_address(
	...
	if (vxcp->vxc_vc->vc_pubkey_base) {
		EC_POINT_add(pgroup, pubkey, pubkey,
			vxcp->vxc_vc->vc_pubkey_base, vxcp->vxc_bnctx);
	}
...
void vg_output_timing_console(vg_context_t *vcp, double count,
	...
	if (vcp->vc_chance >= 1.0) {
		...
		if (prob <= 0.999) {
			p = snprintf(
				&linebuf[p], rem, "[Prob %.1f%%]", prob * 100);
			assert(p > 0);
			rem -= p;
			if (rem < 0) rem = 0;
		...
		for (unsigned i = 0; i < sizeof(targs) / sizeof(targs[0]);
			i++) {
			targ = targs[i];
...
void vg_output_match_console(vg_context_t *vcp, EC_KEY *pkey,
	...
	if (vcp->vc_pubkey_base) {
		...
		EC_POINT_add(EC_KEY_get0_group(pkey), ppnt, ppnt,
	...
	if (isscript)
		vg_encode_script_address(ppnt, EC_KEY_get0_group(pkey),
			vcp->vc_istestnet, addr2_buf);
	else {
		if (isaddresscompressed) {
			vg_encode_compressed_address(ppnt,
				EC_KEY_get0_group(pkey), vcp->vc_istestnet,
				addr_buf);
		} else {
			vg_encode_address(ppnt, EC_KEY_get0_group(pkey),
				vcp->vc_istestnet, addr_buf);
		}
	}

	if (isaddresscompressed)
		vg_encode_privkey_compressed(
			pkey, vcp->vc_privtype, privkey_buf);
	else
		vg_encode_privkey(pkey, vcp->vc_privtype, privkey_buf);

	if (vcp->vc_verbose > 0 || !vcp->vc_result_file ||
...
int vg_context_hash160_sort(vg_context_t *vcp, void *buf) {
	if (!vcp->vc_hash160_sort) return 0;
...
static int get_prefix_ranges(int addrtype, const char *pfx, BIGNUM **result) {
	...
	if (addrtype == (1 << 3)) {
		...
		if (pfx[0] != 'q') {
			fprintf(stderr,
				"The first character of the prefix must be 'q' "
				"for "
				"standard addresses.\n");
			ret = -2;
			goto out;
...
static void vg_prefix_free(vg_prefix_t *vp) {
	if (vp->vp_low) BN_free(vp->vp_low);
	if (vp->vp_high) BN_free(vp->vp_high);
	free(vp);
}
...
static void vg_prefix_context_clear_all_patterns(vg_context_t *vcp) {
	...
	while (!avl_root_empty(&vcpp->vcp_avlroot)) {
		vp = avl_item_entry(
			vcpp->vcp_avlroot.ar_root, vg_prefix_t, vp_item);
...
static void vg_prefix_context_next_difficulty(vg_prefix_context_t *vcpp,
	...
	if (vcpp->base.vc_verbose > 0) {
		if (vcpp->base.vc_npatterns > 1)
			fprintf(stderr,
				"Next match difficulty: " COLOR36 "%s" COLOR0
				" (%ld prefixes)\n",
...
static int vg_prefix_context_add_patterns(
	...
	for (i = 0; i < npatterns; i++) {
		...
		ret = get_prefix_ranges(
			vcpp->base.vc_addrtype, patterns[i], ranges);
		if (!ret) {
			vp = vg_prefix_add_ranges(
				&vcpp->vcp_avlroot, patterns[i], ranges, NULL);
		}
	...
	if (!npfx && impossible)
		fprintf(stderr,
			"Advice: run with the -c argument to see the "
			"conditions.\n");
...
research:
	...
	if (vp) {
		...
		if (vcpp->base.vc_remove_on_match) {
			...
			vg_prefix_range_sum(
				vp, vxcp->vxc_bntarg, vxcp->vxc_bntmp);
			BN_sub(vxcp->vxc_bntmp, vcpp->vcp_difficulty,
				vxcp->vxc_bntarg);
			...
			if (!avl_root_empty(&vcpp->vcp_avlroot))
				vg_prefix_context_next_difficulty(vcpp,
...
static void vg_regex_context_clear_all_patterns(vg_context_t *vcp) {
	...
	if (vcrp->vcr_regex_pat) free(vcrp->vcr_regex_pat);
...
static int vg_regex_match_handler(unsigned int id, unsigned long long from,
	...
	if (vcrp->base.vc_remove_on_match) {
		...
		hs_compile_error_t *compile_error;
		if (vg_regex_compile_patterns(vcrp->vcr_regex_pat,
			    vcrp->base.vc_npatterns, &vcrp->vcr_db,
			    &compile_error) != HS_SUCCESS) {
			printf("An error occured while re-compiling patterns: "
			       "%s\n",
				compile_error->message);
			hs_free_compile_error(compile_error);
			rct->state = 2;
...
restart_loop:
	...
	if (scan != HS_SUCCESS) {
		fprintf(stderr,
			"ERROR: Unable to scan for regex. Error code: %d.\n",
			scan);
